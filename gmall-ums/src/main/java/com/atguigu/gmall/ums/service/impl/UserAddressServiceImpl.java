...
public class UserAddressServiceImpl extends ServiceImpl<UserAddressMapper, UserAddressEntity> implements UserAddressService {
    ...
    @Override
    public List<UserAddressEntity> queryUserAddress(Long userId) {
        List<UserAddressEntity> userAddressList = this.list(new QueryWrapper<UserAddressEntity>().eq("user_id", userId));
